...
class XYZ(BaseQubit):
    ...
    default_options = Dict(
        ...
        pocket_width='650um',
        pocket_height='650um',
        GDS = True, # ***If true, then will be used for GDS. If False, will be used for HFSS analysis
        double = False, # ***(double = False) -> Single Junction, and (double = True) -> double junction
        dx = '10um',
    ...
    def make_pocket(self):
        ...
        # pad_bot = draw.translate(pad, 0, -(pad_height + pad_gap) / 2.) # ***Commenting Out
        ...
        # *** Drawing the Rectangle Representing the Josephson Junction for either GDS or HFSS formats
        if GDS == True:
            # ===================
            # GDS Export
            # ===================

            # Single Junction Case
            rect_jj = draw.rectangle(p.inductor_width, pad_gap)
            
            # Double Junction Case
            rect_jj_R = draw.translate(rect_jj, dx + p.inductor_width/2,0)
            rect_jj_L = draw.translate(rect_jj, -dx - p.inductor_width/2,0)
        else:
            # ===================
            # HFSS Export
            # ===================
            ...
            # Double Junction Case
            rect_jj_R = draw.LineString([(dx + p.inductor_width/2, -pad_gap /2), (dx+p.inductor_width/2, +pad_gap/2 )])
            rect_jj_L = draw.LineString([(-dx - p.inductor_width/2, -pad_gap/2), (-dx - p.inductor_width/2, +pad_gap/2 )])


        rect_pk = draw.rectangle(p.pocket_width, p.pocket_height)
        rect_pk = draw.translate(rect_pk,0, p.pocket_height/2 - pad_gap/2) # ***Shifting the pocket upwards such that the bottom of the pocket borders the JJ(s)
        ...
        if double == False:
            polys = [rect_jj, pad_top, rect_pk] # *** Removing 'pad_bot'
        else:
            polys = [rect_jj_R, rect_jj_L, pad_top, rect_pk] # *** Removing 'pad_bot'

        polys = draw.rotate(polys, p.orientation, origin=(0, 0))
        polys = draw.translate(polys, p.pos_x, p.pos_y)

        if double == False:
            [rect_jj, pad_top , rect_pk] = polys # *** Removing 'pad_bot'
        else:
            [rect_jj_R, rect_jj_L, pad_top , rect_pk] = polys # *** Removing 'pad_bot'


        # Use the geometry to create Metal qgeometry
        self.add_qgeometry('poly', dict(pad_top=pad_top)) # *** Removing 'pad_bot = pad_bot'
        ...
        if GDS == True:
            if double == False:
                ...
                self.add_qgeometry('poly', dict(
                    rect_jj_R=rect_jj_R), helper=True)
                self.add_qgeometry('poly', dict(
                    rect_jj_L=rect_jj_L), helper=True)            
        else:
            if double == False:
                ...
                self.add_qgeometry('junction',
                                dict(rect_jj_R=rect_jj_R),
                                width=p.inductor_width)
                self.add_qgeometry('junction',
                                dict(rect_jj_L=rect_jj_L),
                                width=p.inductor_width)                
