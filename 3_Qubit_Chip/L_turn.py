...
class L_turn(QComponent):
    ...
    def make(self):
        ...
        second_y = second_y = - p.second_gap - p.second_width / 2 #        second_y = second_y = -p.prime_width / 2 - p.prime_gap - p.coupling_space - p.second_gap - p.second_width / 2
        ...
        c_items = [second_cpw, second_cpw_etch]
        c_items = draw.rotate(c_items, p.orientation, origin=(0, 0))
        c_items = draw.translate(c_items, p.pos_x, p.pos_y)
        [second_cpw, second_cpw_etch] = c_items
        ...
        self.add_pin('second_end',
                     points=np.array(second_pin_list[1:]),    #points=np.array(second_pin_list[1:]),
                     width=p.second_width,
                     input_as_norm=True)
